...
from env.env_helpers import setup_maniskill_env
from env.env_helpers import fetch_rgb_from_obs
...
class EvalAgent:
    ...
    def test(self, model: PreTrainedPolicy, custom_instruction=None, verbose=True):
        """
        ...
        if custom_instruction is None:
            ...
            instructions = list([custom_instruction]*self.num_envs)
            print(f"Custom instruction: {instructions}")
...
def main(cfg_eval: DictConfig):
    ...
    # start evaluation
    try:
        eval_agent.test(model, custom_instruction=cfg_eval.get('custom_instruction', None), verbose=cfg_eval.verbose)
