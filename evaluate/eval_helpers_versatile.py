...
from ManiSkill.mani_skill.utils.visualization.misc import images_to_video
...
class BatchEpisodeData:
    ...
    def get_env_data(self, env_idx: int) -> dict:
        ...
        return {
            ...
            "proprioception": self.proprioception[env_idx],
...
def create_batch_episode_data(num_envs: int) -> BatchEpisodeData:
    """Create a new BatchEpisodeData instance"""
    return BatchEpisodeData(num_envs)
...
def save_batch_episode_data(batch_data: BatchEpisodeData, data_dir: Path):
    ...
    batch_summary = {
        ...
        "all_proprioception": batch_data.proprioception,  # [env_idx][step] = proprioception_list (already converted to lists),
        "all_instructions": batch_data.instructions,  # [env_idx][step] = instruction_list (already converted to lists),
...
def create_batch_videos(batch_data: BatchEpisodeData, video_dir: Path, fps: int = 10, filter_success: bool = False, custom_filename: str = ""):
    ...
    print(f"Creating videos: {batch_data.num_envs} total environments")

    if filter_success:
        success_rate = np.mean(batch_data.success)
        num_successful = sum(batch_data.success)
        print(f"{num_successful} successful ({success_rate:.2%})")
    
    # 1. Create regular tiled video (all environments)
    tiled_video_data = batch_data.get_tiled_video_data()
    if not filter_success:
        filename_regular = custom_filename if custom_filename else "tiled_all_envs"
    else:
        filename_regular = custom_filename if custom_filename else f"tiled_all_envs_success_rate_{success_rate:.2f}"
    ...
    if tiled_video_data["images"]:
        ...
        print(f"filename_regular={filename_regular}")
