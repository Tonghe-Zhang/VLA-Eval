...
project_root = quick_test_dir.parent.parent  # Project/
...
from env.test.forward_kinematics import forward_kinematics
...
class BaseEnvTester:
    ...
    def setup_config(self):
        ...
        self.proprioception_type = self.cfg.get("proprioception_type", "ee_pose") # "qpos"
    ...
    def get_rgb_obs_with_info_overlay(self, obs, info, reward):
        ...
        if self.use_render_camera: # obtained from env.render()
            ...
            obs_rgb_overlay = fetch_rgb_from_obs_allenvs(
                self.env_type, obs, self.sim_device, self.model_device, info=info, reward=reward, normalize=False
            )
    ...
    def run(self):
        ...
        # Initial observation
        obs_rgb_overlay = self.get_rgb_obs_with_info_overlay(obs, info, reward)
